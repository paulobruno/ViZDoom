...
def player1():
    ...
    save_path_player1 = save_path + 'player1/'

    if save_log:
        make_sure_path_exists(save_path_player1)
        if load_model:
            log_file = open(save_path_player1+log_savefile, 'a')
        else:
...
def player2():
    ...
    def perform_learning_step(eps):
        ...
        game.make_action(actions[a], frame_repeat)
        current_frags = game.get_game_variable(GameVariable.FRAGCOUNT)
        reward = reward_multiplier * (current_frags - last_frags)
    ...
    if save_log:
        make_sure_path_exists(save_path_player2)
        if load_model:
            log_file = open(save_path_player2+log_savefile, 'a')
        else:
            log_file = open(save_path_player2+log_savefile, 'w')
            print('Map: ' + game_map, file=log_file)
            print('Resolution: ' + str(game_resolution), file=log_file)
            print('Image channels: ' + str(img_channels), file=log_file)
            print('Frame repeat: ' + str(frame_repeat), file=log_file)
    ...
    if load_model:
        make_sure_path_exists(save_path_player2+model_savefile)
        print('P2: Loading model from: ', save_path_player2+model_savefile)
        saver.restore(sess, save_path_player2+model_savefile)
    else:
        init = tf.global_variables_initializer()
        sess.run(init)
    ...
    if not skip_learning:
        print('P2: Starting the training!')
        ...
        for epoch in range(num_epochs):
            ...
            for _ in range(train_episodes_per_epoch):
                ...
                score = reward_multiplier * game.get_game_variable(GameVariable.FRAGCOUNT)
#                score = game.get_total_reward()
                train_scores.append(score)
                train_episodes_finished += 1
                
#                print("Episode finished!")
#                print("player2 frags:", score)
...
#                game.make_action(choice(actions))
            ...
            train_scores = np.array(train_scores)
            print('P2 Results: mean: %.1f±%.1f,' % (train_scores.mean(), train_scores.std()), \
                  'min: %.1f,' % train_scores.min(), 'max: %.1f,' % train_scores.max())
            ...
            for test_episode in range(test_episodes_per_epoch):
                ...
                r = reward_multiplier * game.get_game_variable(GameVariable.FRAGCOUNT)
#                r = game.get_total_reward() 
                test_scores.append(r)
            ...
            test_scores = np.array(test_scores)
            print('P2 Results: mean: %.1f±%.1f,' % (test_scores.mean(), test_scores.std()), \
                  'min: %.1f,' % test_scores.min(), 'max: %.1f,' % test_scores.max())
        ...
        # Starts a new episode. All players have to call new_episode() in multiplayer mode.
            ...
            if save_model:
                make_sure_path_exists(save_path_player2+model_savefile)
                print('P2 Saving the network weights to:', save_path_player2+model_savefile)
                saver.save(sess, save_path_player2+model_savefile)
...
#    raw_input('Press Enter to continue...') # in python3 use input() instead
    ...
    video_index = 1
    make_sure_path_exists(save_path_player2 + "records")

    for i in range(episodes_to_watch):
        game.clear_game_args()
        game.add_game_args("-join 127.0.0.1")
        game.add_game_args("+name Player2 +colorset 3")
        game.add_game_args("-record " + save_path_player2 + "records/ep_" + str(video_index) + "_rec.lmp")
        video_index += 1
